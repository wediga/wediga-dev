...
// The quiet landing. A two-track layout: the public content reads down a
// left-aligned column in an ordinary scroll, while the sun sits on the other
// track and stays in view as you read. The same teaser, toolkit and access door
// as the ride, so both appearances say the same thing. The canvas hosts the
// engine's quiet render mode (the sun alone, cursor-reactive, no camera ride).
export function QuietLanding({
  ...
  return (
    <div className="relative min-h-[100dvh] px-[max(28px,6vw)]">
      {/* The sun. On wide screens it is fixed, centred in the right half (centre
      ...
      {/* Content. The text column sits in the left half, anchored to its right
          ...
          half. Text is centre-aligned within the column, not pushed around in the
          half. Clears the pinned footer at the foot. */}
      <div className="relative z-10 pb-[16vh] pt-[4vh] xl:flex xl:w-1/2 xl:justify-end xl:pb-[20vh] xl:pr-[3vw] xl:pt-[15vh]">
        {/* Below xl the sun sits above the content, so the column is centred under
            it (a readable measure, not pinned far left on a tablet). At xl the
            two-track takes over and these reset. */}
