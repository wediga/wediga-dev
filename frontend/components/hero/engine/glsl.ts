// Motion model: spring to a slowly moving target. Each atom is pulled toward its
// home on a body whose centre orbits the sun and whose surface offset spins, so
// the system keeps circling without any gravity integration.

// Velocity pass: spring ramps in as the impulse wave reaches the atom, plus an
// idle swirl before forming and a soft cursor push.
...
// Dave Hoskins sin-free hash; a sin hash lost precision at large indices and made
...
void main() {
  ...
  // Open: the docked planet's atoms gather into a camera-facing disc backing the
  // DOM text. r via sqrt fills uniformly, so it reads as a solid disc not a ring.
  float open = 0.0;
  ...
  // The impulse is a wave: an atom springs only once uForm passes its per-atom
  // delay, so the sun resolves first and the rim last.
  float act = smoothstep(m.x, m.x + 0.9, uForm);

  vec3 accel = (target - pos) * (14.0 * act);

  // Before its turn the atom drifts so the raw cloud stays alive.
  float idle = 1.0 - act;
  accel += vec3(-pos.z, pos.x * 0.15, pos.x) * 0.04 * idle;

  // Cursor as a camera ray; atoms push by perpendicular distance to it, so the
  // influence reaches every depth and orbit height, including the tilted outer
  // planets the old flat y = 0 projection missed.
...
// Position pass: integrate the velocity just written this frame.
...
// Render pass: lit points reading the computed position texture. Look uniforms
// (size, softness, intensity) are set once at build time.
...
void main() {
  // Quiet mode shows the sun alone (bodyId -1); planet atoms collapse off screen
  // at zero size so they never rasterise. Full motion leaves uSunOnly at 0.
  ...
  vOpen = isOpen * uOpen;
  // Everything except the docked planet steps back while text is open.
  vDim = mix(mix(1.0, 0.16, uDim), 1.0, isOpen);
  vec4 mv = modelViewMatrix * vec4(pos, 1.0);
  gl_Position = projectionMatrix * mv;
  // Atoms stay fine grains even up close and while a planet opens; the open disc
  // covers the background through density and opacity (see fragment), not by
  // fattening into blobs.
...
void main() {
  ...
  // Shade and a bright centre make each grain read as a lit point, not a flat dot.
  float lum = mix(0.72, 1.05, vShade);
  float centre = 1.0 + (1.0 - d) * 0.35;
  // The opened planet's grains turn near-opaque so the disc backs the text; the
  // rest dims back.
